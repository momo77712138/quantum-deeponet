...
def main():
    ...
    def branch_transform(x):
        ...
        mask = branch_max != branch_min
        d = x.shape[1]
        x_scaled = 2 * (x - branch_min) / (branch_max - branch_min) - 1  # Rescale to [-1, 1]
        x_scaled = np.where(mask, x_scaled, x)
        x_scaled = x_scaled / np.sqrt(d)
        x_d1 = np.sqrt(1 - np.sum(x_scaled**2, axis=1, keepdims=True))
        return np.concatenate((x_scaled, x_d1), axis=1)
    ...
    losshistory, training_status = model.train(iterations=40000,disregard_previous_best = True)
